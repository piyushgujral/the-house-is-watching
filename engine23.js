...
function boot(){
 ...
 // Room-specific architecture: every objective area gets a recognizable landmark.
 // Entry: coat bench + luggage; Study: desk/shelves; Bedroom: bed frame; Ritual: altar; Back: crates.
 ...
 // Wall frames create strong visual depth without textures/assets.
 ...
 // Procedural evidence marks: scratches and dragged trails point toward the ritual room.
 ...
 // Canvas signs are crisp, lightweight and do not require external art.
 ...
 // Route breadcrumbs: faint marks pulse after the Doll, visually guiding exploration without becoming a waypoint arrow.
 ...
 g.updatePlayer=function(dt){oldPlayer(dt);if(this.state!=='PLAYING')return;elapsed+=dt;nextEvent-=dt;const p=this.player,sc=S;const x=p.x/sc,z=p.z/sc;const r=z>7?'ENTRY':z>0?'HALL':z>-7?(x<0?'BEDROOM':'STUDY'):z>-13?'RITUAL':'BACK';if(r!==lastRoom){lastRoom=r;if(g.campaign&&g.campaign.stage>=2&&Math.random()<.55){const lines={ENTRY:'THE FRONT DOOR IS CLOSER THAN IT FEELS',HALL:'THE HOUSE HEARD THAT',BEDROOM:'SOMEONE SLEPT HERE RECENTLY',STUDY:'THE PAPERS ARE STILL WARM',RITUAL:'THE AIR IS HOLDING ITS BREATH',BACK:'THERE IS NOWHERE LEFT TO HIDE'};say(lines[r]);}}
  if(g.campaign&&g.campaign.stage>=4){const active=g.campaign.flags&&g.campaign.flags.doll;breadcrumbs.forEach((b,i)=>{b.visible=!!active;b.material.emissive&&b.material.emissive.setHex(0x3a0b0b);b.material.opacity=.45+Math.sin(elapsed*2+i)*.18;b.material.transparent=true;});}
 ...
 // Final-chase correction: Engine 22 intentionally unifies the AI, but its generic hunting speed
 // can overwrite Engine 18's 3.7 speed. Once all seals are broken, this layer restores the actual finale.
 const oldEntity=g.updateEntity.bind(g);
 g.updateEntity=function(dt){oldEntity(dt);if(this.state!=='PLAYING')return;const final=(this.phase>=6)||(this.houseEndgame&&this.houseEndgame.finished);if(final&&this.entity){this.entity.state='CHASE';this.entity.speed=3.7;const w=this.entityGroup,p=this.player;if(w&&p){const dx=p.x-w.position.x,dz=p.z-w.position.z,d=Math.hypot(dx,dz)||1;w.position.x+=dx/d*3.7*S*dt;w.position.z+=dz/d*3.7*S*dt;w.rotation.y=Math.atan2(dx,dz);w.position.x=Math.max(-11.2*S,Math.min(11.2*S,w.position.x));w.position.z=Math.max(-14.1*S,Math.min(14.1*S,w.position.z));if(d/S<1.15)this.die();}}};
 // Give the front-door extraction a short readable transition before victory.
