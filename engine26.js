...
function boot(){
 ...
 const root=document.createElement('div');root.id='coop-foundation-hud';root.style.cssText='position:fixed;left:18px;top:112px;min-width:205px;padding:9px 11px;background:rgba(4,4,6,.55);border:1px solid rgba(210,190,170,.2);font:700 10px/1.55 monospace;letter-spacing:1.2px;color:#d9cabb;z-index:34;pointer-events:none;display:none';root.innerHTML='<div style="font-size:9px;opacity:.55;letter-spacing:2px">HOUSE PARTY</div><div id="coop-members"></div><div id="coop-objective" style="margin-top:5px;opacity:.72"></div>';
 ...
 function revive(id){const m=members.get(id);if(!m||m.state!=='DOWNED')return false;m.state='ALIVE';m.health=45;m.revive=0;state.reviveCount=(state.reviveCount||0)+1;state.downedCount=Math.max(0,state.downedCount-1);if(g.worldOverhaul&&g.worldOverhaul.say)g.worldOverhaul.say(m.name+' IS BACK ON THEIR FEET');return true;}
 ...
 function nearestTarget(){let best=null,bd=Infinity,s=g.scene.scale&&g.scene.scale.x||1;members.forEach(m=>{if(m.state==='DEAD')return;if(m.id==='local'){best=m;return;}const p=m.object&&m.object.position;if(!p)return;const d=Math.hypot(g.player.x-p.x,g.player.z-p.z)/s;if(d<bd){bd=d;best=m;}});return best||local;}
 function render(){
  const el=root.querySelector('#coop-members'),obj=root.querySelector('#coop-objective');
  el.innerHTML=Array.from(members.values()).map(m=>{const label=m.state==='DOWNED'?'DOWN':m.state==='DEAD'?'LOST':m.name;return '<div>'+label+' <span style="opacity:.45">'+m.room+'</span></div>';}).join('');
  const c=g.campaign;e=g.houseEndgame;
  obj.textContent=e&&e.started&&!e.finished?'SEALS: '+(e.seals||0)+'/3':c?('STAGE '+c.stage):state.mode;
  root.style.display=(members.size>1||state.mode!=='SOLO_READY')?'block':'none';
 }
 // Public API is transport-neutral: a future server can feed addRemotePlayer/updateRemotePlayer.
 ...
 g.updatePlayer=function(dt){
  oldUpdatePlayer(dt);if(this.state!=='PLAYING')return;syncCampaign();updateRevives(dt);render();
  if(local.state==='DOWNED'){local.revive-=dt;if(local.revive<=0)down('local');}
 };
 g.updateEntity=function(dt){
  oldUpdateEntity(dt);if(this.state!=='PLAYING'||!this.entity)return;
  const target=nearestTarget();
  // Target selection hook: remote players become valid future network targets,
  // while the current prototype remains authoritative over the local avatar.
  if(target&&target.id!=='local'&&this.entity.state==='HUNTING'&&this.entity.targetId!==target.id)this.entity.targetId=target.id;
  if(this.entity.state==='CHASE'&&local.state==='ALIVE')this.entity.targetId='local';
 };
